...
import { queryListWorkerDefinition, queryRawScript } from '@/service/modules/projects-worker-definition'
import type { IJsonItem } from '../types'

export function useWorkerDefinition(model: { [field: string]: any }): IJsonItem {
	const { t } = useI18n()
	...
	const getWorkerGroups = async () => {
		...
		await queryListWorkerDefinition({
			pageSize: 100, pageNo: 1, searchName: '', userId: 2,
		}).then((res: any) => {
